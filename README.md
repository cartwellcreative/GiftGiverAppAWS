...
```bash
gift-giver$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
gift-giver$ sam local start-api
gift-giver$ curl http://localhost:3000/
```
...
```bash
gift-giver$ sam logs -n HelloWorldFunction --stack-name gift-giver --tail
```
...
```bash
sam delete --stack-name gift-giver
```
